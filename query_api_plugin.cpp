...
public:
   ...
   const void get_token_contracts( url_response_callback &&cb )
   {
      auto blog = _ctrl.block_log();
      ...
      for ( auto i = first_block_num; i <= head_block_num; ++i )
      {
         ...
         {
            if ( v.trx.contains<packed_transaction>() )
            {
               const auto &tx = v.trx.get<packed_transaction>().get_transaction();
